...
Edge::PluginHandleReference Edge::PluginController::loadPlugin(const FileName& moduleName, PluginType pluginType, const char* generatorName)
{
	...
	const PluginID pluginID = PluginBase::calcPluginID(moduleID, pluginType);
	auto pluginIter = m_plugins.find(pluginID);
	if (pluginIter == m_plugins.end())
	{
		IPlugin* plugin = m_loader->loadPlugin(moduleInfo.m_moduleHandle, moduleID, generatorName);
		if (!plugin)
		{
...
void Edge::PluginController::unloadPlugin(IPlugin* plugin)
{
	assert(m_loader);
	assert(plugin);
