...
TS_D3D11::D3D11Texture2D::D3D11Texture2D(const TS::Texture2DDesc& desc, const D3D11DeviceComPtr& device)
	: Texture2D(desc)
{
...
TS_D3D11::D3D11Texture2D::D3D11Texture2D(const TS::Texture2DDesc& desc, const D3D11Texture2DComPtr& texture)
	: Texture2D(desc), m_d3d11Texture(texture)
{
	createViews(desc);
}

void TS_D3D11::D3D11Texture2D::createViews(const TS::Texture2DDesc& desc)
{
	if (!m_d3d11Texture)
	{
		return;
	...
	if (desc.m_usage & TS::TEXTURE_USAGE_RENDER_TARGET)
	{
	...
	if (desc.m_usage & TS::TEXTURE_USAGE_DEPTH_STENCIL)
	{
